...
class Pokemon {
    ...
    Pokemon(int id, int generation, String name, String description, ArrayList<String> types,
            ArrayList<String> abilities, double weight, double height, int captureRate, boolean legendary, Date captureDate) {
        this.id = id;
    ...
    public void read(String s) throws Exception {
        ...
        boolean hasComma = aux[6].contains(",");
...
public class Q13 {
    static void merge(Pokemon[] arr, int left, int mid, int right, TP tp) {
        int n1 = mid - left;
        int n2 = right - mid;

        Pokemon[] L = new Pokemon[n1];
        Pokemon[] R = new Pokemon[n2];
        ...
        while (i < n1 && j < n2) {
            int compare = compareTypes(L[i], R[j]);
            if (compare <= 0) {
                arr[k] = L[i];
                i++;
            } else {
    ...
    static void mergeSort(Pokemon[] arr, int left, int right, TP tp) {
        if (left < right - 1) {
            int mid = (left + right) / 2;
            mergeSort(arr, left, mid, tp);
            mergeSort(arr, mid, right, tp);
            merge(arr, left, mid, right, tp);
        }
    }

    static int compareTypes(Pokemon a, Pokemon b) {
        String typeA = a.getTypes().isEmpty() ? "" : a.getTypes().get(0);
        String typeB = b.getTypes().isEmpty() ? "" : b.getTypes().get(0);
