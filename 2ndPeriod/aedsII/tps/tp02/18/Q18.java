...
public class Q18{
	...
	static void quickSort(Pokemon[] arr, int left, int right, Question q){
		int i = left,
			j = right;
		...
		if(left<j)
			quickSort(arr, left, j, q);
		if(i<right)
			quickSort(arr, i, right, q);
	}
	...
	static void insertion(Pokemon[] arr, Question q){
		int n = arr.length;
		for(int i=1; i<n; i++){
			Pokemon tmp = arr[i];
			int j = i-1;
	...
	static void sort(Pokemon[] pokes, Question q, int k){
		quickSort(pokes, 0, pokes.length-1, q);
		insertion(pokes, q);
	}
