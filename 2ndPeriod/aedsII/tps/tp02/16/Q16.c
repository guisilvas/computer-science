...
void logTP(const char* fileName, double time, int comps, int moves){
    ...
    if(fil){
        fprintf(fil, "ID: 863485\t");
        fprintf(fil, "Execution Time: %lf\t", time);
        fprintf(fil, "Comparisons: %d\t", comps);
        fprintf(fil, "Movements: %d", moves);
        fclose(fil);
    }
...
int main(){
    ...
    free(using);
    free(pokes);
    logTP("863485_insertion.txt", diff(start, end), comps, moves);
